...
useSeoMeta({
  ...
  ogTitle: "Documents Storage",
  description: "Upload and manage your document storage.",
  ogDescription: "Easily store and organize documents for your system.",
});
...
const { storageFiles, getAllFiles, uploadFile, deleteFile } = useDocumentStorage();

const isDeleteModalOpen = ref(false);

const selectedDocumentName = ref<any>(null);
...
const handleDelete = async () => {
  ...
  await deleteFile(selectedDocumentName.value);
  dataDetail.name = null;
  dataDetail.size = 0;
  dataDetail.updated_at = null;
  dataDetail.document = null;
...
<template>
  <div class="w-full mx-auto my-16 space-y-[30px] max-w-7xl">
    <div class="flex justify-between items-center mb-4">
      <h1 class="text-xl font-semibold">Documents Storage</h1>
      <div>
        <UButton
          class="h-10"
          color="white"
